/*
Назначение: Перечесление всех сообщений системы
...
public enum Message
{
    ...
    ConnectionError = 1, // Ошибка подключения (нет подключения к сети интернет)
    ProtocolError = 2, // Неверный метод отправки запроса
    UndefinedError = 3, // Неизвестная ошибка
    ServerError = 4, // Ошибка на сервере
    ...
    CouldNotParseResponse = 112, // Не удалось десериализовать ответ от сервера, возможно там есть лишние данные,
                                 // или какие-то из них равны null, хотя не должны (например, числа)
    // БД
    ...
    TestCanNotOpenedAndClosedTogether  = 504, // Тест не может быть одновременно открыт и закрыт
